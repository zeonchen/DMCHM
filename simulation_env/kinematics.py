...
class Vehicle(RoadObject):
    ...
    @classmethod
    def create_random_merge(cls, road: Road,
                              speed: float = None,
                              lane_from: Optional[str] = None,
                              lane_to: Optional[str] = None,
                              lane_id: Optional[int] = None,
                              spacing: float = 1, add=False) \
            -> "Vehicle":
        """
        ...
        """
        lane_dict = list(road.network.graph.keys())[:-2]
        _from = lane_from or road.np_random.choice(lane_dict)
        ...
        if add:
            ...
            if lane_from == 'a':
                previous_speed = 10
                cur_lane_idx = (_from, _to, _id)
                for vehicle in road.vehicles:
                    v_l_id = vehicle.lane_index
                    
                    if v_l_id == cur_lane_idx:
                        speeds.append(vehicle.speed)
                    v_lc = vehicle.position[0]
                    if v_lc < min_loc:
                        min_loc = v_lc
            ...
            spacing = 0.5 if lane_from == 'j' else 0.3
            default_spacing = 30 + 3 * speed
            offset = spacing * default_spacing * np.exp(-5 / 40 * len(road.network.graph[_from][_to]))

            x0 = min(3 * offset, min_loc)
            x0 -= offset * road.np_random.uniform(.9, 1.1)
            x0 = min(x0, 0.5)
    ...
    def step(self, dt: float, time) -> None:
        """
        ...
        if cur_fv:
            cur_fv_state = np.concatenate(cur_fv.past_record[-1:]).mean(axis=0)
            sv_state = np.concatenate(self.past_record[-1:]).mean(axis=0)
        
        gap = cur_fv_state[0] - sv_state[0] if cur_fv else 0

        new_record = np.array([[self.position[0], v[0], self.action['acceleration'], gap, time]])
        self.past_record.append(new_record)
